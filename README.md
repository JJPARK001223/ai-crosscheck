...
Codex에 작업을 요청하면 상위 세션이 다음 명령과 같은 방식으로 전체 절차를 실행한다.

```powershell
.\ask.ps1 "질문 또는 작업 내용"
```

`orchestrate.ps1`은 독립 조사 → 교차검증 → Claude Code 최종 종합을 수행하며, 실행 중 60초마다 단계와 결과 파일 상태를 점검한다. 최종 사용자 답변은 Claude Code가 `FINAL.md`에 작성한다.
