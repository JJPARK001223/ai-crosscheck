...
function Sync-Repo($msg, [string[]]$paths) {
  & git.exe add @paths
...
# claude(foreground) + codex(background job) 동시 실행
function Invoke-Pair($claudePrompt, $codexPrompt) {
  $job = Start-Job -ScriptBlock {
    param($repo, $prompt)
    Set-Location $repo
    $last = Join-Path $repo '.codex_last.txt'
    & codex exec -C $repo -s workspace-write --skip-git-repo-check -o $last $prompt 2>&1 | Out-Null
    $code = $LASTEXITCODE
...
Reset-Round
Sync-Repo "라운드 시작: PROBLEM.md 갱신" @('PROBLEM.md', 'round1', 'rounds')
...
Show-Result $d
Sync-Repo "DECISION.md 초안" @('DECISION.md', 'round1')
